...
export default function ChangePasswordForm({ setShowModal, toastRef }) {
    ...
    const onSubmit = async() => {
        ...
        if(!resultUpdatePassword.statusResponse) {
            setErrorNewPassword("Hubo un problema cambiando la contraseña, por favor intenta más tarde.")
            return
        }
    ...
    const validateForm = () => {
        ...
        if(size(newPassword) < 6) {
            setErrorNewPassword("Debes ingresar nueva contraseña de al menos 6 carácteres.")
            isValid = false
        }
        ...
        if(newPassword !== confirmPassword) {
            setErrorNewPassword("La nueva contraseña y la nueva confirmación no son iguales.")
            setCurrentConfirmPassword("La nueva contraseña y la nueva confirmación no son iguales.")
            isValid = false
        }
        
        if(newPassword === currentPassword) {
            setCurrentErrorPassword("Debes ingresar una contraseña diferentes al actual.")
            setErrorNewPassword("Debes ingresar una contraseña diferentes al actual.")
            setCurrentConfirmPassword("Debes ingresar una contraseña diferentes al actual.")
            isValid = false
        }
